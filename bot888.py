...
TRADE_CONFIG = {
    'target_symbols': [
        ...
        'DOGE/USDT:USDT', 'LINK/USDT:USDT', 'MATIC/USDT:USDT'
    ],
...
class DeepSeekTradingBot:
    ...
    def _analyze_all_symbols(self):
        ...
        for symbol in TRADE_CONFIG['target_symbols']:
            try:
                ...
                # 过滤低信心度信号
                if signal_data['confidence'] < 0.51:
                    logger.info(f"⭐ {symbol} 信心度不足: {signal_data['confidence']:.1%}")
                    continue
    ...
    def _calculate_composite_score(self, strategy_analysis, signal_data):
        ...
        try:
            ...
            rr_bonus = min(20, (rr_ratio - 2) * 10) if rr_ratio > 2 else 0
            ...
            composite_score = (
                ...
                ai_confidence * 100 * 0.4 +     # AI信心度占40%
                rr_bonus +                      # 风险回报比加分
                consistency_bonus               # 策略一致性加分
            )
    ...
    def _select_best_opportunity(self, opportunities):
        ...
        # 检查是否达到最低分数要求
        if best_opportunity['composite_score'] < 75:
            logger.info(f"💤 最佳机会评分{best_opportunity['composite_score']:.1f}低于75分，放弃交易")
            return None
...
命令:
  python ds77.py              # 正常启动（30分钟周期）
  python ds77.py --test       # 测试模式（不实际下单）
  python ds77.py --single     # 单次分析
  python ds77.py --monitor    # 仅监控持仓
  python ds77.py --stats      # 输出实时统计
  python ds77.py --report     # 生成日报
  python ds77.py --help       # 显示帮助
        ...
        for pos in positions:
            if float(pos.get('contracts', 0)) > 0:
                ...
                position_data = {
                    ...
                    'pnl_percent': 0,
                    'stop_loss': trading_state.positions.get(symbol, {}).get('stop_loss', 0),
                    'leverage': float(pos.get('leverage', 1))
                }
                
                # 计算盈亏百分比
                if position_data['entry_price'] > 0:
                    position_data['pnl_percent'] = position_data['unrealized_pnl'] / (
                        position_data['entry_price'] * position_data['quantity']
                    )
                
                # 检查并更新移动止损
                trailing_stop_manager.check_and_update_trailing_stop(symbol, position_data)
                
                # 更新到全局状态
                trading_state.positions[symbol] = position_data
...
# ==================== 🎯 增强交易执行（集成小资金优化） ====================
def enhanced_trade_execution(symbol, signal_data, market_data, available_balance):
    """增强版交易执行 - 集成小资金优化"""
    try:
        # 动态调整杠杆
        leverage = PortfolioManager().calculate_dynamic_leverage(signal_data, market_data)
        signal_data['leverage'] = leverage
        
        # 计算仓位比例
        position_percent = signal_data['position_size_percent']
        ...
        if quantity <= 0:
            logger.warning("⚠️ 仓位计算无效，跳过交易")
            return False
        
        logger.info(f"✅ {symbol} 仓位计算完成: {quantity:.8f}")
        return quantity
...
# ==================== 📝 使用示例 ====================
def example_usage():
    """使用示例 - 演示100U同时持有BTC和DOGE"""
    
    print("\n" + "="*70)
    print("📝 小资金多币种持仓示例（100 USDT）")
    print("="*70)
    ...
    planned_positions = [
        {
            ...
            'entry_price': 110000.0,  # BTC价格
            'position_size_percent': 0.30,  # 30%资金
            'leverage': 10
        },
        {
            'symbol': 'DOGE/USDT:USDT',
            'entry_price': 0.1985,  # DOGE价格
            'position_size_percent': 0.40,  # 40%资金
            'leverage': 8
        }
    ]
    
    print(f"\n💰 可用余额: ${available_balance:.2f} USDT")
    print(f"📊 计划开仓币种数: {len(planned_positions)}")
    
    # 计算每个仓位
    calculated_positions = []
    for plan in planned_positions:
        position_value = available_balance * plan['position_size_percent']
    ...
    # 验证资金分配
    print(f"\n🔍 验证资金分配:")
    if small_capital_optimizer.validate_multi_position_allocation(
        available_balance, calculated_positions
    ):
        print("✅ 资金分配合理，可以同时开仓")
    else:
        print("❌ 资金不足，需要调整仓位")
...
决策原则：
1. 只在综合评分>65分且信心度>0.51时才建议BUY(买入) / SELL(卖出) / HOLD(观望)
2. 基于技术分析建议合理的止损止盈价位，确保风险回报比≥2.5:1
...
"""
    ...
    def _create_fallback_signal(self, market_data, strategy_analysis):
        ...
        signal = 'HOLD'
        if score > 70 and confidence > 0.6 and direction == 'long':
            signal = 'BUY'
        elif score > 70 and confidence > 0.6 and direction == 'short':
            signal = 'SELL'
...
class TradingExecutor:
    ...
    def _pass_risk_checks(self, symbol, signal_data):
        ...
        # 信心度检查
        if signal_data['confidence'] < 0.51:
            logger.warning(f"⚠️ 信心度不足: {signal_data['confidence']:.2%}")
            return False
        ...
        if rr_ratio < TRADE_CONFIG['risk_management']['risk_reward_ratio']:
            logger.warning(f"⚠️ 风险回报比不足: {rr_ratio:.2f}")
            return False
